...
4. Inicie a coleta manual quando precisar atualizar a base e processar feeds:
...
5. Para manter o polling recorrente em execução:
...
3. A home abre já autenticada como administrador no modo de desenvolvimento; a tela de login continua acessível somente pela URL padrão de autenticação.
...
7. O processamento das notícias fica em [core/services/noticia_service.py](core/services/noticia_service.py), que faz o parsing, filtra relevância, salva notícias e cria alertas.
...
9. O usuário usa [core/views.py](core/views.py), [core/services/chat_service.py](core/services/chat_service.py) e os templates em `templates/core/` para navegar entre home, dashboard e chat.
...
- [core/views.py](core/views.py) e `templates/` implementam a experiência web autenticada e o chatbot.
...
- O auto-login do administrador no `runserver` é controlado por `DJANGO_AUTO_LOGIN_ADMIN`.
...
- O chatbot fica em `/chat/` e usa a mesma configuração de OpenAI do projeto, com fallback local quando a chave não está disponível.
